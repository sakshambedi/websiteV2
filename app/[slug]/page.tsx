            ...
            <article className="flex flex-col flex-nowrap w-full  items-center phone:prose-figure:w-full ">
                <div className="flex flex-col w-full items-center bg-sky-700 dark:bg-sky-400 h-72 justify-end ">
                    <div className="flex flex-col w-6/12  phone:w-9/12 md:w-3/4 ">
                        <h1 className="flex font-rebondG  text-4xl justify-start mb-0 pt-14 pb-3  text-white dark:text-black phone:text-lg phone:pt-6 phone:pb-2">{blogPost.title}</h1>
                        <p className='flex font-mono text-base justify-start my-0 pb-10 text-gray-200 dark:text-gray-800'>Published Date: {blogPost.date?.toLocaleDateString()}</p>
                    </div>
                </div>

                <div id="content" className="flex flex-col w-6/12 phone:w-9/12 md:w-3/4 ">
                    <div className="w-full pt-2 phone:pt-5" dangerouslySetInnerHTML={{ __html: blogPost.contentHtml }} />
